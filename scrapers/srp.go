...
// srpExtractCfg is the shared extraction config for the browser / fallback path.
// ScraperVersion is filled in at scrape-time.
var srpExtractCfg = PageExtractConfig{
	...
	BaseURL:        "https://www.srpnet.com",
}

// SRPScraper discovers and scrapes rebate programs from srpnet.com.
//
// SRP is behind Cloudflare which blocks data-centre IP ranges at the CDN
// level (confirmed: direct curl returns HTTP 403 + server:cloudflare).
// This scraper therefore always uses a headless Chromium instance (go-rod)
// for both sitemap discovery and page visits.  Rod auto-downloads Chromium
// on first use (~150 MB, cached at ~/.cache/rod/browser/).
type SRPScraper struct {
	CollyBase
	ScraperVersion string
	Logger         *zap.Logger
	HTTPClient     *http.Client // optional override (tests only)
...
//
// SRP is behind Cloudflare which blocks data-centre IPs regardless of
// User-Agent — this scraper always uses headless Chromium for both sitemap
// discovery and page visits.
func (s *SRPScraper) Scrape(ctx context.Context) ([]models.Incentive, error) {
	// Always start the headless browser — direct HTTP never works for SRP.
	bf, err := newBrowserFetcher(s.Logger)
	if err != nil {
		return nil, fmt.Errorf("srp: start headless browser: %w", err)
	}
	defer bf.Close()

	// Step 1: discover rebate URLs from sitemap via browser.
	// Skip the plain HTTP attempt entirely — SRP's CDN always blocks it.
	allURLs, sitemapErr := FetchSitemapURLsBrowser(ctx, bf, srpSitemapURL)
	var urls []string
	if sitemapErr != nil || len(allURLs) == 0 {
		if sitemapErr != nil {
			s.Logger.Warn("srp: browser sitemap fetch failed, using seed URLs", zap.Error(sitemapErr))
		}
		urls = srpSeedURLs()
	} else {
	...
	for i, u := range urls {
		...
		if fetchErr != nil {
			s.Logger.Warn("srp: browser fetch failed", zap.String("url", u), zap.Error(fetchErr))
			bar.Add(1) //nolint:errcheck
			continue
		}
		...
		if parseErr != nil {
			s.Logger.Warn("srp: parse failed", zap.String("url", u), zap.Error(parseErr))
			bar.Add(1) //nolint:errcheck
			continue
		}
...
// extractPage retains the Colly-based extraction for reference / test use.
// Not called during normal operation — SRP always uses the browser path.
